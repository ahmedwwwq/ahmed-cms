        ...
        {currentTab === 'home' && (
          <div className="space-y-16 animate-fadeIn">
            ...
            <section className="relative rounded-3xl overflow-hidden bg-slate-900 border border-[#C9A14A]/25 min-h-[440px] sm:min-h-[500px] flex items-center justify-center text-center p-6 py-12 sm:p-12 sm:py-20 shadow-2xl">
              ...
              <div className="relative z-10 max-w-5xl space-y-16 sm:space-y-28 w-full">
            ...
            {/* Primary Service Cards grid (أصبحت الآن خارج الصورة الخلفية الرئيسية للواجهة) */}
                  ...
                  bgImage: 'https://lh3.googleusercontent.com/d/1rh_xhhHGEUNNTFr52vT0vRzI1R1sjNj9',
                    ...
                    return (
                      <motion.div
                        ...
                        whileTap={{ scale: 0.97 }}
                        className="bg-slate-950/60 border border-white/10 hover:border-[#C9A14A]/60 rounded-[2rem] p-6 sm:p-7 flex flex-col items-center justify-between text-center min-h-[190px] sm:min-h-[220px] transition-all duration-300 cursor-pointer group relative overflow-hidden backdrop-blur-xl"
                      >
                        ...
                        {/* Overlay with subtle gold hue and deep dark fallback to ensure contrast */}
                        <div className="absolute inset-0 bg-gradient-to-b from-slate-950/70 via-slate-950/65 to-slate-950/80 group-hover:from-slate-950/60 group-hover:via-slate-950/50 group-hover:to-slate-950/65 transition-colors duration-500 z-0" />
                        ...
                        <motion.div 
                          ...
                          className="w-14 h-14 rounded-2xl bg-slate-900/80 border border-white/10 text-[#C9A14A] flex items-center justify-center mb-4.5 group-hover:border-[#C9A14A]/40 group-hover:bg-[#C9A14A]/15 transition-all duration-300 shadow-md shrink-0 relative z-10 overflow-hidden"
                        >
                          {/* Inner glowing core */}
                          ...
                          {serv.id === 'student_housing' ? (
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-8 h-8 drop-shadow-[0_2px_10px_rgba(201,161,74,0.4)] transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
                              {/* The House base */}
                            ...
                            <IconComponent className="w-7 h-7 sm:w-8 sm:h-8 drop-shadow-[0_2px_10px_rgba(201,161,74,0.4)] transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6" />
                          )}
                        </motion.div>

                        {/* Content text matching layout */}
                        <div className="space-y-1.5 relative z-10 w-full flex-grow flex flex-col justify-center">
                          <h3 className="text-xs sm:text-sm md:text-[15px] font-black text-white leading-tight font-arabic group-hover:text-[#C9A14A] transition-colors duration-300">
                            {isRtl ? serv.ar : serv.en}
                          </h3>
                          <p className="text-[10px] sm:text-xs text-gray-300 font-medium font-arabic leading-relaxed group-hover:text-white transition-colors duration-300">
            ...
            <section className="space-y-6">
              <div className="flex items-end justify-between">
                <div className="flex items-center gap-3">
                  <span className="w-1.5 h-8 sm:h-10 bg-[#C9A14A] rounded-full shrink-0"></span>
                  <div>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-[#163B63] dark:text-white font-arabic">
                      {lang === 'ar' ? 'العقارات المميزة والأكثر طلباً' : 'Featured Premium Collections'}
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-light">
                      {lang === 'ar' ? 'تم اختيارها بعناية لتفي بمتطلباتكم من الفخامة والأناقة' : 'Curated premium listings cataloged exclusively'}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => {
                    setCurrentTab('family_rentals');
                    window.scrollTo({ top: 100, behavior: 'smooth' });
                  }}
                  className="text-xs font-bold text-[#C9A14A] hover:underline whitespace-nowrap"
                >
                  {lang === 'ar' ? 'عرض الكل' : 'View All'}
                </button>
              </div>

              {/* Slider / Grid of properties */}
