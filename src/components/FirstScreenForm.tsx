...
import { Globe, Phone, User, Home, ArrowRight, ShieldCheck } from 'lucide-react';
...
export const FirstScreenForm: FC<FirstScreenFormProps> = ({ onComplete }) => {
  ...
  const [service, setService] = useState('student_housing');
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-lg border text-left text-[11px] transition-all flex items-center justify-between ${
                      service === s.id
                        ? 'bg-[#C9A14A]/20 border-[#C9A14A] text-[#C9A14A] font-semibold'
                        : 'bg-white/5 border-white/10 text-white hover:bg-white/10'
                    }`}
                    dir={isRtl ? 'rtl' : 'ltr'}
                  >
                    <span className="truncate pr-1">{lang === 'ar' ? s.arName : s.enName}</span>
                    <div className={`w-3 h-3 rounded-full border flex items-center justify-center shrink-0 ${
                      service === s.id ? 'border-[#C9A14A] bg-[#C9A14A]' : 'border-gray-500'
                    }`}>
                      {service === s.id && <div className="w-1 h-1 bg-[#163B63] rounded-full" />}
                    </div>
                  </button>
                ))}
              </div>
